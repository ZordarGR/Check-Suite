...
| `reports.js` | the redacted departures print AND the redacted Boarding List (`mealplandetail`, 1.17.56): a boarding page laid out like his 07/09 file (names replaced) — recognised, the header and day totals read, and the four row shapes (a plain room, a name glued to the room with the room peeled back off, an adjoining-room range `414-15`, an unallocated `?`) redacted to a dash with room and counts kept, no fixture name on the sheet. Plus the departures print: the Departure List by Time parser over a page laid out exactly like his 06/09/26 file (names replaced), and the sheet. The columns come from the file's own heading line; the room type beside the number is read apart; the groups carry their time, the first one none; a note under a room is kept and the fragment protel prints in the guest column beside it is withheld; the totals are read; the sheet carries no guest name and no fragment, and everything else; a checkcharge page is not a departure list; and his 08/09 file's three ways a row went missing (1.17.58): a `427-2` room 7.68 left of its heading is a row as printed, a row whose numbers sit 0.64 under its text beneath a clipped name fragment comes out whole, page 2's repeated header stays out of page 1's last room; the exact print's parts (1.17.59): which runs and characters the parser withheld and where each dash goes, a font part unscrambled by its GUID, a glyph's implicit advance from the font's own hmtx, protel's rectangles and clips drawn with a clip id unique per page, every withheld character cut and nothing else, an element the renderer cannot draw reported rather than dropped, a token in the name band on a line the parser could not place cut too; the file-delete door removes only a report file inside the reports folder, to the Recycle Bin |
| `browser/exact.js` | the EXACT PRINT (1.17.59) through the real page: REPORTS' preview of a departure list that carries its pages shows protel's two pages as SVG with the names cut and a note beside the paper saying how many characters went; Print arms the job and the PDF is exactly two A4 landscape pages edge to edge, no blank third; no fixture name in the preview or on the print root; a list without pages still prints the tabular sheet, landscape with its margins; the corrections after it are portrait and the zero margin is gone |
...
`legacy.js`, `ledger.js`, `live.js`, `alerts.js`, `movesfit.js`, `reports.js`, `exact.js`,
`orient.js`, `dbgshot.js`, `optshot.js`.
