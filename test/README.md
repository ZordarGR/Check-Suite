...
| `status.js` | the STATUS store, through the shipped writer and marks: each list kept for the day as the union of its captures; an arrival gone from the arrival list is nothing until the in-house list shows the same name and room with CI; a departure gone from the departure list is nothing until a COMPLETE in-house list captured afterwards does not show it — a cut-short one proves nothing; `414` and `414-15` are one room; the pills come from the store and from nothing else, and the dot lands on a departed, turnover or moved pill only under that reservation's own name — over the whole stay since 1.17.44, from the room+name pairs each loaded report leaves behind by night; with legacy ON the XPS-fed ledger draws them instead, as before 1.17.42 |
