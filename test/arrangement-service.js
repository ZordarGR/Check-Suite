...
(async()=>{
 ...
 controller.setEnabled(false);assert.equal(window.visible,false);assert.equal(commands.at(-1),"scope 1 skip\n");
 send(invoice);ipcMain.emit("arrangement-painted",{sender:window.webContents},paid);assert.equal(window.visible,false,'off ignores invoice/paint packets');
 const pausedHistory=fs.readFileSync(path.join(dir,'arrangement-rates-v1.json'),'utf8');
 assert(controller.ingestCapture('IH',txt,sourceAt+=10));assert.equal(window.visible,false,'capture history continues without an overlay');
 const afterPaused=JSON.parse(fs.readFileSync(path.join(dir,'arrangement-rates-v1.json'),'utf8'));
 assert.deepEqual(afterPaused.map(x=>({...x,at:0})),JSON.parse(pausedHistory).map(x=>({...x,at:0})), 'paused capture preserves all reservation facts');
 assert.equal(afterPaused[0].at,sourceAt,'paused capture saves the newer observation');
 controller.setEnabled(true);assert.equal(commands.at(-1),"scope 1 read 1\n");
 send(invoice);assert.notEqual(packet.result.state,'paid','old in-flight invoice cannot restore green');
 send({kind:'scope',epoch:1,read:true,request:0});send(invoice);assert.notEqual(packet.result.state,'paid','old read acknowledgement is not a resume acknowledgement');
 send({kind:'scope',epoch:1,read:true,request:1});send(invoice);assert.equal(packet.result.state,'paid');
 controller.setEnabled(false);controller.setEnabled(true);controller.setEnabled(false);controller.setEnabled(true);
 assert.equal(commands.at(-1),'scope 1 read 3\n');
 send({kind:'scope',epoch:1,read:true,request:2});send(invoice);assert.notEqual(packet.result.state,'paid','rapid toggles reject an earlier resume');
 send({kind:'scope',epoch:1,read:true,request:3});send(invoice);assert.equal(packet.result.state,'paid');
 ...
 const pausedApp=new EventEmitter();let spawned=0;
 const paused=start({electron:{app:pausedApp,ipcMain:new EventEmitter(),BrowserWindow:Win,screen:{screenToDipRect:(_w,r)=>r}},helperPath:'unused',captureDir:dir,userData:dir,enabled:false,spawnHelper:()=>{spawned++;throw Error('disabled reader launched');}});
 assert.equal(spawned,0);assert(paused.ingestCapture('IH',txt,Date.now()));assert.equal(spawned,0);pausedApp.emit('before-quit');
