...
const v="1.17.70",p=JSON.parse(fs.readFileSync("app/package.json"));p.version=v;
fs.writeFileSync("app/package.json",JSON.stringify(p,null,2)+"\n");
const html=fs.readFileSync("app/index.html","utf8").replace(/APP_VERSION\s*=\s*"1\.17\.69"/,'APP_VERSION = "'+v+'"');
...
fs.writeFileSync("dist-win64/nsi/reccheck.nsi",fs.readFileSync("dist-win64/nsi/reccheck.nsi","utf8").replace('!define VERSION "1.17.63"','!define VERSION "'+v+'"'));
NODE
...
const v="1.17.70",m=JSON.parse(fs.readFileSync("update/latest.json"));
...
notes:"Live accommodation payment check in Invoice B: exact totals, over/under differences, and a faint red tint when unpaid. Uses list Price and agency, counts advance payments, handles the extra late-arrival night, and follows the Invoice as it moves or resizes. Original reservation dates and Department/Tax Check data are preserved."});
