/* The arrival and departure reports feeding the ledger, through the SHIPPED functions.
   ...
   IT LIVES UNDER THIS NAME BECAUSE IT WAS ONCE DESTROYED. It was `test/reports.js` until
   1.17.50 gave the redacted departures print the same filename and wrote over it; all 51
   assertions below went with it, `run.sh` kept running the survivor twice, and the README
   kept a row for each — so the repo went on claiming a guard it no longer had. Nothing
   else in test/ touches `feedStays`. Found and restored 06/09, unchanged and still green.
   Do not name a harness after a screen; name it after the thing it holds the line on.
