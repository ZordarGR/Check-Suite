...
(async()=>{
 ...
 try{
  const source=await get('https://raw.githubusercontent.com/ZordarGR/Check-Suite/6282e7645756b9f8ed2382d563ac3ee56cc11c6c/app/updater.js');
  ...
  const pending=await u.check({forcedOnly:true});assert(pending&&pending.forced===true);assert(pending&&!pending.full);assert.equal(pending.version,version);assert(u.promote());
  ...
  console.log('FORCED_HTML_DELIVERY_VERIFIED '+JSON.stringify({version,engine:manifest.engine,sha256:manifest.sha256,bytes:html.length,installed:false}));
